...
def merge_logic(original_csv, results_jsonl, final_csv, id_prefix):
    """Merges LLM results back into the original CSV based on index."""
    ...
    print(f"Successfully merged! Saved to: {final_csv}")

# --- CORE PROCESSING ---

def run_pipeline(task_name, input_csv, batch_input_jsonl, batch_output_jsonl, final_csv, prompt, label_map):
    # 1. Condition: If output results exist, MERGE
    if os.path.exists(batch_output_jsonl):
        print(f"[{task_name}] Found results file. Proceeding to merge...")
        merge_logic(input_csv, batch_output_jsonl, final_csv, task_name)
        return

    # 2. Condition: If batch input exists, WAIT for API
    if os.path.exists(batch_input_jsonl):
        print(f"[{task_name}] Batch input file already exists at {batch_input_jsonl}. Please upload to OpenAI and place results in {batch_output_jsonl}")
        return

    # 3. Otherwise: GENERATE batch input
