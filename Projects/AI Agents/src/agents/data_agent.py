...
def _build_quality_report(df_raw: pd.DataFrame, df_clean: pd.DataFrame) -> DataQualityReport:
    ...
    warnings = []
    if missing_dict: warnings.append(f"Missing values in: {list(missing_dict.keys())}")
    if outlier_count > 0: warnings.append(f"{outlier_count} sales outliers (z > 3.5 σ)")
    
    duplicates = int(df_raw.duplicated().sum())
    if duplicates: warnings.append(f"{duplicates} duplicate rows removed")
